...
--        (62, 'JavaBean должен реализовывать интерфейс Serializable, или Externalizable','ни в чем', 'JavaBean является методом, а POJO - классом', 'в POJO отсутствует реализация методов equals и hashCode', 'В чем основное различие между POJO и JavaBean?'),
...
--        (91, 'setter','getter', 'recipient','giver', 'Специальный метод для изменения значения поля объекта, доступ к которому напрямую ограничен'),
...
--        (134, 'внутренний (вложенный класс)','абстрактный класс','enum','класс по умолчанию', 'Класс, полностью объявленный в теле другого класса или интерфейса?'),
--        (135, 'JavaBeans','интерфейс','абстрактный класс','POJO', 'Классы, которые инкапсулируют множество объектов в один объект, они сериализуемы, имеют конструктор без аргументов и разрешают доступ к свойствам с помощью методов получения и установки?'),
--        (136, 'интернационализация','локализация','глобализация','приватизация', 'Процесс разработки программного приложения, позволяющего адаптировать его к различным языкам и регионам без технических изменений:'),
--        (137, 'статический импорт','динамический импорт','нестатический импорт','локальный импорт', 'Функция, которая позволяет использовать элементы (поля и методы) в рамках их контейнерного класса как public static в Java-коде без указания класса, в котором было определено поле?'),
--        (138, 'filter()','map()','distinct()','sorted()', 'Конвейерный метод класса Stream, который  отфильтровывает и возвращает записи, соответствующие условию?'),
--        (139, 'skip()','map()','distinct()','sorted()', 'Конвейерный метод класса Stream, который позволяет пропустить N первых элементов?'),
--        (140, 'distinct()','skip()','map()','sorted()', 'Конвейерный метод класса Stream, который возвращает стрим без дубликатов?'),
--        (141, 'map()','distinct()','skip()','sorted()', 'Конвейерный метод класса Stream, который преобразует каждый элемент стрима?'),
--        (142, 'peek()','map()','distinct()','sorted()', 'Конвейерный метод класса Stream, который возвращает тот же стрим, но применяет функцию к каждому элементу стрима?'),
--        (143, 'limit()','map()','distinct()','sorted()', 'Конвейерный метод класса Stream, который позволяет ограничить выборку определенным количеством первых элементов?'),
--        (144, 'sorted()','limit()','map()','distinct()','Конвейерный метод класса Stream, который позволяет сортировать значения либо в натуральном порядке, либо задавая Comparator?'),
--        (145, 'findFirst()','findAny()','collect()','count()','Терминальный метод класса Stream, который возвращает первый элемент из стрима (возвращает Optional)?'),
--        (146, 'collect()','findAny()','findFirst()','count()','Терминальный метод класса Stream, который представляет результат в виде коллекций и других структур данных?'),
--        (147, 'anyMatch()','noneMatch()','allMatch()','findAny()','Терминальный метод класса Stream, который возвращает true, если условие выполняется хотя бы для одного элемента?'),
--        (148, 'noneMatch()','anyMatch()','allMatch()','findAny()','Терминальный метод класса Stream, который возвращает true, если условие не выполняется ни для одного элемента?'),
--        (149, 'allMatch()','noneMatch()','anyMatch()','findAny()','Терминальный метод класса Stream, который возвращает true, если условие выполняется для всех элементов?'),
--        (150, 'min()','max()','forEach()','forEachOrdered()','Терминальный метод класса Stream, который возвращает минимальный элемент, в качестве условия использует компаратор?'),
--        (151, 'max()','min()','forEach()','forEachOrdered()','Терминальный метод класса Stream, который возвращает максимальный элемент, в качестве условия использует компаратор?'),
--        (152, 'forEach()','max()','min()','forEachOrdered()','Терминальный метод класса Stream, который применяет функцию к каждому объекту стрима, не гарантируя сохранение порядка элементов при параллельном выполнении?'),
--        (153, 'forEachOrdered()','forEach()','max()','min()','Терминальный метод класса Stream, который применяет функцию к каждому объекту стрима, гарантируя сохранение порядка элементов?'),
--        (154, 'toArray()','forEach()','anyMatch()','map()','Терминальный метод класса Stream, который возвращает массив значений стрима?'),
--        (155, 'reduce()','toArray()','forEach()','anyMatch()','Терминальный метод класса Stream, который позволяет выполнять агрегатные функции на всей коллекцией и возвращать один результат?'),
--        (156, 'sum()','reduce()','toArray()','forEach()','Метод класса Stream, который возвращает сумму всех чисел?'),
--        (157, 'average()','sum()','reduce()','toArray()','Метод класса Stream, который возвращает среднее арифметическое всех чисел?'),
--        (158, 'mapToObj()','average()','reduce()','toArray()','Метод класса Stream, который преобразует числовой стрим обратно в объектный?'),
--        (159, 'isParallel()','parallel()','reduce()','sequential()','Метод класса Stream, который позволяет узнать является ли стрим параллельным?'),
--        (160, 'parallel()','isParallel()','reduce()','sequential()','Метод класса Stream, который позволяет вернуть параллельный стрим и, если стрим уже параллельный, то возвращает самого себя'),
--        (161, 'sequential()','parallel()','isParallel()','reduce()','Метод класса Stream, который позволяет вернуть последовательный стрим и, если стрим уже последовательный, то возвращает самого себя'),
--        (162, 'String pool','String stack','String rull','String heap','Набор строк, который хранится в памяти Java heap'),
--        (163, 'одну ссылку','три ссылки','не являются объектами','не имеют ссылок (примитивы)','Переменные: String name1 = "ok"; String name2 = "ok"; String name3 = "ok";   - имеют :'),
--        (164, 'экономить память путем присваивания ссылок на одинаковые строки вместо создания объектов','создавать новые объекты и структурировать их в памяти','удалять пустые объекты типа String','объединять строки в одку','Основное предназначение Sting poll (пула строк)'),
--        (165, 'конкатенация','инкапсуляция','дуоризация','сложение','Термин, обозначающий соединение (слияние) строк?'),
--        (166, 'создается новый объект в памяти','переменная обновляется','создается два связных объекта','объект преобразуется в новый класс StringBuilder','При добавлении строки к уже имеющейся переменной типа String ...'),
--        (167, 'переменная обновляется','создается новый объект в памяти','создается два связных объекта','объект преобразуется в новый класс String','При добавлении строки к уже имеющейся переменной типа StringBuilder ...'),
--        (168, 'переменная обновляется','создается новый объект в памяти','создается два связных объекта','объект преобразуется в новый класс String','При добавлении строки к уже имеющейся переменной типа StringBuffer ...'),
--        (169, 'да, null добавиться к строчке в виде продолжения строки ("... null")','нет, будет исключение NullPointerException','нет, будет исключение RuntimeException','нет, программа аварийно завершит работу','Можно ли через бинарный арифметический оператор "+" добавить к строке null'),
--        (170, 'нет, будет исключение NullPointerException','да, null добавиться к строчке в виде продолжения строки ("... null")','нет, будет исключение RuntimeException','нет,программа аварийно завершит работу','Можно ли через метод concat() добавить к строке null'),
--        (171, 'length()','join()','count()','replace()','Метод класса String, который возвращает число символов в строке (длину строки)?'),
--        (172, 'toCharArray()','join()','substring()','replace()','Метод класса String, который возвращает строку в виде массива символов char[]?'),
--        (173, 'isEmpty()','join()','substring()','replace()','Метод класса String, который определяет является ли строка пустой?'),
--        (174, 'concat()','+','substring()','replace()','Метод класса String, который объединяет строки?'),
--        (175, 'valueOf()','join()','substring()','indexOf()','Метод класса String, который преобразует объект в строковый вид?'),
--        (176, 'compare()','join()','substring()','indexOf()','Метод класса String, который сравнивает две строки?'),
--        (177, 'charAt()','join()','substring()','indexOf()','Метод класса String, который возвращает символ строки по индексу?'),
--        (178, 'getChars()','charAt()','substring()','indexOf()','Метод класса String, который возвращает группу символов?'),
--        (179, 'equals()','equalsIgnoreCase()','substring()','replace()','Метод класса String, который сравнивает строки с учетом регистра?'),
--        (180, 'equalsIgnoreCase()','equals()','substring()','replace()','Метод класса String, который сравнивает строки без учета регистра?'),
--        (181, 'indexOf()','valueOf()','join()','substring()','Метод класса String, который возвращает индекс по первому искомому значению?'),
--        (182, 'lastIndexOf()','indexOf()','valueOf()','substring()','Метод класса String, который возвращает индекс последнего искомого значения?'),
--        (183, 'startsWith()','lastIndexOf()','valueOf()','endsWith()','Метод класса String, который определяет, начинается ли строка с указанного значения?'),
--        (184, 'endsWith()','startsWith()','lastIndexOf()','valueOf()','Метод класса String, который определяет, заканчивается ли строка указанным значением?'),
--        (185, 'trim()','substring()','lastIndexOf()','valueOf()','Метод класса String, который удаляет начальные и конечные пробелы?'),
--        (186, 'substring()','trim()','lastIndexOf()','valueOf()','Метод класса String, который возвращает строку, начиная с определенного индекса до конца или до определенного индекса?'),
--        (187, 'toLowerCase()','toUpperCase()','lastIndexOf()','equalsIgnoreCase()','Метод класса String, который переводит все символы строки в нижний регистр?'),
--        (188, 'toUpperCase()','toLowerCase()','lastIndexOf()','equalsIgnoreCase()','Метод класса String, который переводит все символы строки в верхний регистр?'),
--        (189, 'enum','constants','String','args','Набор именованных констант, который помогает в определении своих собственных типов данных?'),
--        (190, 'enum Class{}','static enum Class{}','private enum Class{}','final enum Class{}','В каком примере правильно обозначен enum класс?'),
--        (191, 'да, все перечисленное','только переменные (константы)','только конструктор и константы','только методы и константы','Может ли класс enum иметь конструкторы, методы и переменные?'),
--        (192, 'ссылочного','примитивного','приватного','публичного','Какого типа является Enum в java?'),
--        (193, 'статичными и финализированными','статичными','приватными','финализированными','Константы Enum в java неявно (по-умолчанию) являются...'),
--        (194, 'EnumMap и EnumSet','EnumArray и EnumList','EnumList и EnumSet','EnumMap и EnumArray','Типы коллекций для работы с enum?'),
--        (195, 'нет (не требуется), константы в enum по умолчанию статичны','да','нет, enum является интерфейсом','нет, enum является абстрактным классом','Нужно ли создать экземпляр Enum с помощью оператора "new"?'),
--        (196, 'EnumMap и EnumSet','EnumArray и EnumList','EnumList и EnumSet','EnumMap и EnumArray','Типы коллекций для работы с enum?'),
--        (197, 'может реализовывать только интерфейс (интерфейсы)','может реализовывать интерфейс и унаследовать свойства других классов','может только унаследовать свойства других классов','не может','Может ли класс enum реализовывать интерфейсы и унаследовать свойства других классов??'),
--        (198, 'name()','toString()','ordinal()','compareTo()','Метод Enum, который возвращает задекларированное имя константы?'),
--        (199, 'toString()','name()','ordinal()','compareTo()','Метод Enum, который по умолчанию возвращает задекларированное имя константы, может быть переопределен?'),
--        (200, 'ordinal()','toString()','name()','compareTo()','Метод Enum, который возвращает порядковый номер в котором экземпляр перечисления обозначен внутри enum?'),
--        (201, '0', '5', '50', '5.0', 'Каким будет ответ выражения двух чисел типа "int": (10 % 2)?'),
--        (202, '5', '0', '2', '5.0', 'Каким будет ответ выражения двух чисел типа "int": (10 / 2)?'),
--        (203, '5.0', '0.0', '5', '50.0', 'Каким будет ответ выражения двух чисел типа "double": (10.0 / 2.0)?'),
--        (204, '1.0', '3.0', '1', '3', 'Каким будет ответ выражения двух чисел типа "double": (10.0 % 3.0)?'),
--        (205, '1.0', '3.0', '1', '3',  'Каким будет ответ выражения двух чисел типа "double": (5.0 % 2.0)?'),
--        (206, 'Рефлексия', 'Инкапсуляция', 'Истерия', 'Фреймворкинг', 'Механизм, с помощью которого можно вносить изменения и получать информацию о классах, интерфейсах, полях и методах во время выполнения, при этом не зная имен этих классов, методов и полей?'),
--        (207, 'Перегрузка', 'Переопределение', 'Перереализация', 'Переписывание', 'Приём программирования, который позволяет в одном классе для методов с разными параметрами использовать одно и то же имя?'),
--        (208, 'аналогичное названию класса', 'как и название класса, но в нижнем регистре', 'как и название класса, но в верхнем регистре', 'любое', 'Какое название имеет конструктор в классе?'),
--        (209, '@Documented', '@Target', '@Inherited', '@Retention','Аннотация, которая указывает, что элементы, использующие эту аннотацию должны быть задокументированы в Javadoc и аналогичных инструментах?'),
--        (210, '@Retention','@Target','@Documented', '@Inherited', 'Аннотация, которая указывает, как долго сохранять аннотации с аннотированными типами?'),
--        (211, '@Inherited', '@Retention','@Target','@Documented','Аннотация, которая указывает, что тип аннотации автоматически передается по наследству?'),
--        (212, '@Target','@Inherited', '@Retention','@Documented','Аннотация, которая указывает виды элементов программы, к которым применим тип аннотации. Возможные значения: TYPE, METHOD, CONSTRUCTOR, FIELD и т.д. ?'),
--        (213, '@Deprecated','@Inherited', '@Retention','@Target','Аннотация, которая указывает, что используемый метод является устаревшим?'),
--        (214, 'Паттерн','Шаблон', 'Функциональный метод','JavaBean','Cтандартизированное решение какой-либо часто встречающейся проблемы в программировании?'),
--        (215, 'Singleton','Multiton', 'Reangton','Marafon','Паттерн - класс, у которого есть только один объект (один экземпляр класса) и к этому объекту предоставляется глобальная точка доступа?'),
       (216, 'Кроссплатформенность','Полиморифзм', 'Мультиязычность','Абстракционизм','Свойство (преимущество перед другими языками програмирования) программ на Java, которое означает, что они могут быть запущены на любой платформе без каких-либо изменений?'),
       (217, 'колизия','двойственность', 'такого не бывает','дуализм','Ситуация, когда два объекта имеют одинаковый хешкод (HashCode)?'),
       (218, 'Optional','Integer', 'Stream','String','Объект-контейнер, используемый для хранения ненулевых объектов, а также используется для представления нулевого значения с отсутствующим значением?'),
       (219, 'empty()','of()', 'ofNullable()','filter()','Метод класса Optional, который создает пустой класс Optional?'),
       (220, 'ofNullable()','of()','empty()', 'filter()','Метод класса Optional, который создает опционал, при этом если переданное значение содержит null - пустой оптионал, если содержит значение - объект со сначением?'),
       (221, 'filter()','empty()','of()', 'ofNullable()','Метод класса Optional, который принимает в параметр Predicat и возвращает булево значение, если значение true, то возвращается этот же опционал, если значение false, то возвращается пустой опционал?'),
       (222, 'flatMap()','empty()','ofNullable()','filter()','Метод класса Optional, который применяет функцию к значению оптионал?'),
       (223,  'get()','empty()','of()','filter()','Метод класса Optional, который возвращает значение оптионала?')
