...
--        (169, '1.8','1.11','1.6','1.2', 'В какую версии JAVA добавлен запуск приложений JavaFX (прямой запуск JAR-файлов приложений JavaFX)?'),
       ...
       (227, 'нет (не требуется), константы в enum по умолчанию статичны','да','нет, enum является интерфейсом','нет, enum является абстрактным классом','Можно ли создать экземпляр Enum с помощью оператора "new"?'),
